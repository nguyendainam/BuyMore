...
export default function ListOrder() {
    ...
    return (
        <div className={style.mainViewForm}>
            <div className={style.formContent}>
                ...
                <div className={style.formTable}>
                    <Table dataSource={dataItems}>
                        ...
                        <Column title='Items' render={(value, record) => (<div className={style.items}>{record.quantity} items</div>)} />
                        <Column title='Paid' render={(value, record) => {

                            return (<div className={style.itemPaid}>{record.paid = 'paid' ? 'Yes' : 'No'}</div>)
                        }} />
                        <Column title='Status' render={(value, record) => {
                            let color;
                        ...
                        <Column title='Price' render={(value, record) => (<div>{record.totalPrice.toLocaleString().replace(/,/g, '.')}</div>)} />
                        <Column title='' render={(value, record) => {


                            record.status === 'Accepted' ? <div className={style.icon}
            ...
            {isModalVisible && (
                <Modal title="Change Status" visible={isModalVisible} onCancel={onClose} onOk={handleOnSave}>
                    <div className={style.formInput}>
                        <Select defaultValue={'Processing'}
                            value={selectItem.status}
                            onChange={(value) => handleChangeStatus(value)}
                            options={arrOption} className={style.formRegister} />
                    </div>
                </Modal>
            )}
