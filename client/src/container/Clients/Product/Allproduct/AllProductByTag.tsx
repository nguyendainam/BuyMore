...
import { getAllProductByKey, getAllProductByTag, getAllProductByTas } from '../../../../components/AllProduct';
...
export default function AllProductByTag() {
    const [activeOptions, setActiveOptions] = useState([]);
    const [btnFilter, setbtnFilter] = useState<string>(optionFilterProduct[3].key)
    ...
    const t = useLocation().search.split("?key=")[1]
    const handleGetProduct = async () => {
        const getList = await getAllProductByTag(t)
        ...
        <div className={style.mainView}>
            <div className={style.content}>
                <div className={style.sideMenu}>
                    <div className={style.priceProduct}>
                        <div className={style.priceInput}>
                            <Input className={style.formMoney} value={funds.start.toLocaleString() + 'đ'} />
                            <Input className={style.formMoney} value={funds.end.toLocaleString() + 'đ'} />
                        </div>
                        <Slider
                            ...
                            max={money.end}
                            onChange={onChange}
                            value={[typeof funds.start === 'number' ? funds.start : 0, typeof funds.end === 'number' ? funds.end : 0]}
                        />
                    ...
                    {Select.OptionClient.map((item) => {
                        ...
                        return (
                            <div className={style.formOption} onClick={() => { handleOptionClick(item.value) }}>
                                <div className={style.titleOption} id='Active'
                                ><span>{item.label}</span> <FaAngleDown /></div>
                                <div className={style.option}>
                                    <Checkbox.Group options={Select[item.value]} onChange={(value) => handleFilterProduct(value, item.value)} />
                                </div>
                            </div>
                ...
                <div className={style.listProduct}>
                    <div className={style.title}>Loại Sản Phẩm<span>(80 SẢN PHẨM)</span> </div>
                    <div className={style.Option}><span>Sắp Xếp Theo :</span>
                        {optionFilterProduct.map((item) => {
                            return (
                                <Button

                                    id={item.key === btnFilter ? 'Active' : ''}
                                    onClick={() => handleChangeFillter(item.key)} >{language === 'vi' ? item.labelVI : item.labelEN}</Button>
                            )
                        })}
                    </div>
                    ...
                    <div className={style.products}>
                        {displayedProducts && displayedProducts.length > 0 ?
                            displayedProducts.map((item) => {
                                return (
                                    <div className={style.item} onClick={() => handleDetailProduct(item.idProduct)}>
                                        <div className={style.image}>
                                            <img src={`${URL_SERVER_IMG}${item.image}`} />
                                        </div>

                                        {item.saveMoney > 0 ? (<div className={style.discount}>
                                            <span>Tiết Kiệm</span>
                                            <span>{item.saveMoney.toLocaleString().replace(/,/g, '.')}</span>
                                        </div>) : ''}
                                        ...
                                        <div className={style.price}>
                                            {item.priceShow ? item.priceShow.toLocaleString().replace(/,/g, '.') : item.productPrice.toLocaleString().replace(/,/g, '.')} <u>đ</u>
                                        </div>
                                        {item.saveMoney > 0 ? (<div className={style.oldPrice}>
                    ...
                    <div className={style.pagination}>
                        <Pagination onChange={handlePagination} total={displayedProducts.length} />
                    </div>
