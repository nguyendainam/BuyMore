        ...
        <div className={style.mainListOrder}>
            {listData.length &&
                listData.map((item) => {
                    ...
                    const options = item.Options !== null ? JSON.parse(item.Options) : null;
                    ...
                    const status = statusOrder.filter((status) => status.value === item.StatusCart)
                    ...
                    return (
                        <div className={style.items} key={item.Id}>
                            ...
                            <div className={style.information}>
                                ...
                                <div className={style.nameOption}>{options && resumeData(options)}</div>

                            </div>
                            <div className={style.formPrice}>{item.TotalPrice.toLocaleString()} vnđ</div>
                            <div className={style.formQuantity}><input value={item.Quantity} disabled /></div>
                            <div className={style.formInforOrder}>
                                <div className={style.date}><span>Ngày Đặt Hàng:</span> <span>{moment(item.CreatedAT).format('DD-MM/YYYY HH:MM ')}</span></div>
                                <div className={style.status}><span>Trạng Thái:</span> <span> {language === 'vi' ? status[0].labelVI : status[0].labelEN}</span></div>
