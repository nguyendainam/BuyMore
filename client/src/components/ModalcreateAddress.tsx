...
const AddAddress: React.FC<IAddress> = ({ OpenModal, Onclose, updateAfterSave }) => {
    ...
    const handleOnSave = async () => {
        if (!inforUser.phoneNumber || !inforUser.userName || !inforUser.userDistrict || !inforUser.userWard || !inforUser.userCity || !inforUser.specificAddress) {
            ...
            if (result.data.err === 0) {
                message.success('Add new address successfull')
                setInforUser({
                    userName: '',
                    phoneNumber: '',
                    userCity: '',
                ...
                message.error('add new Addess faild ')
                Onclose()
                setInforUser({
                    userName: '',
                    phoneNumber: '',
    ...
    return (
        ...
        <Modal title="Thêm địa chỉ nhận hàng" open={isModalOpen} className={style.modalCustom} onCancel={Onclose}
            footer={[
                <Button className={style.btnWhite} onClick={Onclose}> Hủy</Button>
                , <Button className={style.btnBlue} onClick={handleOnSave}>Thêm mới địa chỉ</Button>
            ]}
        >
            ...
            <Form layout='inline'>
                ...
                <Form.Item label="Tên" name="username" rules={[{ required: true }]}>
                    <Input prefix={<UserOutlined />} className={style.inputForm}
                ...
                <Form.Item label="Số điện thoại" name="phone number" rules={[{ required: true }]}>
            ...
            />
            <TextArea rows={4} placeholder="Địa chỉ cụ thể"
                maxLength={400}
                style={{ resize: 'none', marginTop: '15px' }}

                value={inforUser.specificAddress}
                onChange={(event) => handleOnchange(event.target.value, 'specificAddress')}
            />
